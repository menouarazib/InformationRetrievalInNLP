...
from utils.utils import go_tokenize, color_predicted_keywords, extract_specific_pos_tags
...
df = pd.DataFrame(df['WordVector'].to_list(), index=df.index, columns=
            [f'WordVector_{i + 1}' for i in range(300)])

keywords = extract_specific_pos_tags(words=tokens, pos_tag_name="JJ")
